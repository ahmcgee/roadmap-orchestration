...
// response caps at ~32k output tokens; below WRITE_CHUNK the prompt is byte-identical to the legacy
// single-write form, above it the payload is split deterministically and FANNED OUT — one Haiku
// writer per part (`<path>.partK`, quoted here-doc, byte-checked) and one assembler (runVerbatim).
