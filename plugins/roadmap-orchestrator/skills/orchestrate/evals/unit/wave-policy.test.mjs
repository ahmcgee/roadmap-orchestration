...
const verifyOk = (extra = {}) => ({ pass: true, blocked: false, failures: [], contractSurfaceTouched: false, diffFiles: [], ...extra })
...
test('load facts: every test lane is told to report loadavg1 + cpuCount, by exact command', async () => {
  const { fn, calls } = makeAgent([{ match: /^verify:a#0$/, result: () => ({ pass: false, blocked: false, failures: ['x'], contractSurfaceTouched: false, diffFiles: [] }) }])
...
test('load facts: a blocked verify degrades with the host load attached', async () => {
  const { fn } = makeAgent([{
    match: /^verify:a#0$/,
    result: () => ({ pass: false, blocked: true, failures: ['runner missing'], contractSurfaceTouched: false, diffFiles: [], loadavg1: 34.5, cpuCount: 16 }),
  }])
...
const sharedRedVerify = (id) => () => ({
  ...
  failingSpecs: [SHARED], contractSurfaceTouched: false, diffFiles: [`src/${id}.ts`],
})
  ...
  const { fn, calls } = makeAgent([
    ...
    { match: /^(verify|gate-verify|opus-gate-verify):beta/, result: sharedRedVerify('beta') },
    {
      ...
      result: () => ({ pass: false, blocked: false, failures: ['calendar-smoke red', 'gamma red'],
        failingSpecs: [SHARED, mine], contractSurfaceTouched: false, diffFiles: ['src/gamma.ts', mine] }),
    },
  ])
...
test('shared red control: a failing spec ONE unit owns in its own diff is not shared', async () => {
  ...
  const { fn } = makeAgent([
    { match: /^(verify|gate-verify|opus-gate-verify):alpha/, result: () => ({ pass: false, blocked: false, failures: ['x'], failingSpecs: [own], contractSurfaceTouched: false, diffFiles: ['src/alpha.ts', own] }) },
    { match: /^(verify|gate-verify|opus-gate-verify):beta/, result: () => ({ pass: false, blocked: false, failures: ['x'], failingSpecs: [own], contractSurfaceTouched: false, diffFiles: ['src/beta.ts', own] }) },
  ])
  ...
  const { fn } = makeAgent([
    { match: /^(plan|replan):a$/, result: planWithFiles(['src/a.ts']) },
    {
      ...
      result: () => {
        ...
        return { pass: round > 2, blocked: false, failures: round > 2 ? [] : ['x'], contractSurfaceTouched: false, diffFiles: ['src/a.ts', ...grew] }
      },
    },
  ])
  const state = await runWave(fn, makePlan([unit('a')]), makeState())
...
test('a clean wave is unchanged: no shared reds, no rulings, no new degradations', async () => {
  ...
  assert.equal(state.degradations, undefined, 'and a clean wave still ledgers nothing')
  assertAllModelsPinned(calls)
})
