...
// evidence caps) and S.chainImplCodex (`summary`/`contractMismatch`/`specGap`/`debt` item caps plus
...
// no codex-chain at all) — so the per-link rules below are what makes the drive non-vacuous.
// =========================================================================================
...
test('harness chain prompts: capped schemas state the length contract', async () => {
  const lane = (await driveChainWave()).filter((c) => /^(lane-setup:|chain-|codex-chain:)/.test(c.label))
  for (const required of ['chain-plan:a', 'codex-chain:a'])
