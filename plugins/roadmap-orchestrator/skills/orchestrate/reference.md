...
- **`degradations`** — **NOT a state.json field.** Degradations are events, not state: each is
  ...
  write-failed | preview-failed | correctness-debt-banked | scope-growth | plan-conflict |
  debt-unbanked | codex-exec |
  ...
  hand-written and the scripts never touch it. Every conductor return carries this run's array,
